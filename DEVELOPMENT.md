...
- 运行记录的 `process_name` 只有在为非空字符串时才直接采用；其它类型和纯空白值按安全名称规则兜底。
- `/api/status` 收集状态异常时返回 `500` JSON：`ok=false`、错误 `msg` 和空 `tasks` 列表；正常响应继续保持任务数组。
...
- `/api/task/action/run/<id>` 缺失任务返回 404；普通业务失败保留 200 + `ok:false`，由调用方读取 `msg`。
...
- 单任务置顶保存失败时返回 `500 + ok:false`，任务文件保持未写入状态。
...
- `/api/task/bulk-action` 通用异常返回 `500 + ok:false` 时仍保留规范化后的 `action/count` 请求上下文。
...
- `/api/backup/delete` 删除已不存在文件时保持幂等成功，返回 `200 + ok:true` 且不调用 `unlink()`。
...
- `/api/logfile/<filename>` 收到非整数 `lines` 时返回 `400 text/plain`，不调用日志读取。
