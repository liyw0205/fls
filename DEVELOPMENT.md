...
- 路由层 UI 组件接入：`/deps`、`/deps/refresh`、`/panel/status`、`/` 仪表盘环境状态、`/about` 面板信息、`/notify/test/<id>` 通知测试结果、`/about/job-log/<id>` 后台任务日志头部卡和 `/about/restart-panel`、`/about/stop-panel` 面板控制结果头部卡渲染、响应式表格 ID 保留与 HTML 转义。
