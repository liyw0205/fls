...
- `/api/task/bulk-action` 保持 `ok/msg` 兼容字段，并返回 `action/count` 及结构化计数；未知批量操作返回 400 且不读取或写入任务。
...
- 任务批量 API、任务动作 API、单任务切换/置顶 API 边界、合集批量/单项边界、合集加入缺失边界、批量未知操作/启用/禁用与取出合集写入边界、兼容运行/停止/删除/置顶入口错误提示。
