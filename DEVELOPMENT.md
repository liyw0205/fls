...
- `/task/new` 和 `/task/edit/<id>` 表单校验失败返回 400，保留表单和安全 back，不写任务文件或重载调度器；编辑缺失任务返回 404。
...
- 任务批量 API、任务动作 API、单任务切换/置顶 API 边界、合集批量/单项边界、批量启用/禁用与取出合集写入边界、兼容运行/停止/删除/置顶入口错误提示。
