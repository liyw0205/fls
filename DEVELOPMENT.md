...
- 路由层 UI 组件接入：`/deps`、`/deps/refresh`、`/panel/status`、`/` 仪表盘环境状态、`/about` 面板信息、`/notify/test/<id>` 通知测试结果、`/about/job-log/<id>` 后台任务日志头部卡、`/about/restart-panel`、`/about/stop-panel` 面板控制结果头部卡、`/online-scripts/log/<id>` 在线脚本安装日志头部卡、`/online-scripts/install/<id>` 安装确认头部卡和 `/online-scripts/install-select/<id>` 安装选择页头部卡渲染、响应式表格 ID 保留与 HTML 转义。
