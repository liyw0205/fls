...
- 固定已保存代理不存在时的 `404` JSON 字段。
- 断言缺失代理请求不会进入真实代理网络检测。
- 验证：目标测试通过；全量测试、响应式烟测、编译检查、JS 语法检查和 diff 检查通过。
...
- 构造内存安装记录，固定日志轮询响应字段。
- 验证 `lines` 参数传入日志读取，内部进程对象不进入响应。
- 测试不访问网络、不启动安装线程或真实任务。
- 验证：目标测试通过；全量测试、响应式烟测、编译检查、JS 语法检查和 diff 检查通过。
...
- 新增日志真实路径校验，页面和 API 共用同一安全边界。
- 指向日志目录外文件的符号链接返回 404，且不会调用日志读取。
- 验证外部文件与链接保持不变。
- 验证：目标测试通过；全量测试、响应式烟测、编译检查、JS 语法检查和 diff 检查通过。
...
- 非整数 `lines` 不再触发 Flask HTML 500，改为稳定 `400` JSON。
- 错误响应保留轮询所需字段，不读取日志、不修改内存任务状态。
- 验证：目标测试通过；全量测试、响应式烟测、编译检查、JS 语法检查和 diff 检查通过。
...
- 非整数 `lines` 改为稳定 `400` JSON，不再返回 HTML 500。
- 参数校验提前到进程状态检查之前，错误请求不读取日志、不修改安装记录。
- 验证：目标测试通过；全量测试、响应式烟测、编译检查、JS 语法检查和 diff 检查通过。
...
- 缺失安装记录的响应补齐 `returncode/error/log_file`，与正常轮询字段对齐。
- 缺失分支保持 HTTP 200 兼容，不解析无关 `lines`、不读取日志或创建状态。
- 验证：目标测试通过；全量测试、响应式烟测、编译检查、JS 语法检查和 diff 检查通过。
...
- 非整数 `lines` 改为 `400 text/plain`，保持日志页面的文本消费协议。
- 参数错误发生在日志读取之前，文件内容保持不变。
- 验证：目标测试通过；全量测试、响应式烟测、编译检查、JS 语法检查和 diff 检查通过。
...
- 固定删除不存在备份文件时的 `200` 成功 JSON。
- 断言检查目标存在性但不调用删除，不启动备份线程或创建归档。
- 验证：目标测试通过；全量测试、响应式烟测、编译检查、JS 语法检查和 diff 检查通过。
...
- 固定代理质量检测接口在代理不存在时的 `404` JSON。
- 断言缺失分支不调用质量检测函数，不执行真实网络访问。
- 验证：目标测试通过；全量测试、响应式烟测、编译检查、JS 语法检查和 diff 检查通过。
