...
- 对照原脏 diff 后确认：任务日志页相关剩余风险主要是运行历史表格、日志链接、安全 `back`、停止 POST 表单和用户可控文本转义缺少页面级回归测试。
...
阶段 25 建议目标：继续低风险收束原长期脏 diff 中尚未覆盖的全局运行历史页筛选、批量 API 状态字段或任务运行历史数据模型边界。
...
3. 可继续补全局运行历史页筛选、批量 API 状态字段、任务运行历史数据模型或错误提示渲染测试。
...
5. 阶段 25 结束时继续更新 `DEVELOPMENT.md`、`docs/DEVELOPMENT_PROGRESS.md`、`docs/SESSION_HANDOFF.md`，并提交推送。
...
请从 `docs/SESSION_HANDOFF.md` 开始，继续阶段 25。优先确认远端和原工作区状态；如继续处理脏文件，只在干净 worktree 中实现并提交，不要直接还原原工作区长期修改。
