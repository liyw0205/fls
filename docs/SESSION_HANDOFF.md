...
生成时间：2026-07-14
当前阶段：阶段 75，运行状态异常 JSON 回归

## 本阶段完成进度

完成度：阶段 75 已完成，自动进入阶段 76。
...
下一步：阶段 76，固定批量任务接口通用异常响应中的请求上下文字段。

1. 在批量接口进入 `try` 前初始化 `action` 和 `task_ids`。
2. 通用 500 响应保留规范化后的 `action/count`。
3. 增加加载任务失败的回归测试，不触发副作用。
...
从本文件继续阶段 76；先固定批量接口通用异常的请求上下文，保持 Flask + 原生 CSS/JS 和无 npm 构建链。
