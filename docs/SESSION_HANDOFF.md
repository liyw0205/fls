...
- 对照原脏 diff 后确认：日志管理页相关剩余风险主要是分组批量删除 UI、单组删除入口、单文件 POST 删除表单和分组名 HTML 转义缺少页面级回归测试。
- 扩展 `tests/test_ui_route_components.py`：
  - 覆盖 `/logs` 页面继续渲染 `log-bulk-toolbar`、分组选框、全选控件和删除选中分组按钮。
...
阶段 23 建议目标：继续低风险收束原长期脏 diff 中尚未覆盖的任务运行历史、批量 API 状态字段或日志查看页返回路径边界。
...
3. 可继续补任务运行历史、批量 API 状态字段、日志查看页 `back` 返回路径或错误提示渲染测试。
...
5. 阶段 23 结束时继续更新 `DEVELOPMENT.md`、`docs/DEVELOPMENT_PROGRESS.md`、`docs/SESSION_HANDOFF.md`，并提交推送。
...
请从 `docs/SESSION_HANDOFF.md` 开始，继续阶段 23。优先确认远端和原工作区状态；如继续处理脏文件，只在干净 worktree 中实现并提交，不要直接还原原工作区长期修改。
