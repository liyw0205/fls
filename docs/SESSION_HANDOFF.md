...
- 对照原脏 diff 后确认：仪表盘历史摘要相关剩余风险主要是首页最近运行和最近异常区域缺少路由级回归测试。
...
- 本阶段只补仪表盘历史摘要回归测试和文档，没有改动仪表盘运行时代码。
...
阶段 28 建议目标：继续低风险收束原长期脏 diff 中尚未覆盖的批量 API 状态字段、任务运行失败历史收尾或错误提示渲染边界。
...
3. 可继续补批量 API 状态字段、任务运行失败历史收尾或错误提示渲染测试。
...
5. 阶段 28 结束时继续更新 `DEVELOPMENT.md`、`docs/DEVELOPMENT_PROGRESS.md`、`docs/SESSION_HANDOFF.md`，并提交推送。
...
请从 `docs/SESSION_HANDOFF.md` 开始，继续阶段 28。优先确认远端和原工作区状态；如继续处理脏文件，只在干净 worktree 中实现并提交，不要直接还原原工作区长期修改。
