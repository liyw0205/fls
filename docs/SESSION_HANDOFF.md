...
- 对照原脏 diff 后确认：普通任务列表相关剩余差异主要是长命令不折叠、停止/置顶/删除等动作退回 GET 链接、编辑/配置入口丢失 `/tasks` back 参数。
- 扩展 `tests/test_ui_route_components.py`：
  - 覆盖 `/tasks` 页面长命令渲染为 `fls-collapsible-code` 折叠代码块，并保留 `fls-value-preview`。
  - 覆盖批量工具栏、批量按钮和 `taskBulkAction('enable')`、`taskBulkAction('delete')` 调用。
  - 覆盖单任务更多菜单和复制、置顶、停止等 `taskAjaxAction(...)` 动作。
...
阶段 22 建议目标：继续低风险收束原长期脏 diff 中尚未覆盖的任务/日志页面行为，或补真实响应式验收。
...
3. 可继续补任务/日志页面中仍未覆盖的 POST 动作、返回路径、折叠展示或错误提示渲染测试。
...
5. 阶段 22 结束时继续更新 `DEVELOPMENT.md`、`docs/DEVELOPMENT_PROGRESS.md`、`docs/SESSION_HANDOFF.md`，并提交推送。
...
请从 `docs/SESSION_HANDOFF.md` 开始，继续阶段 22。优先确认远端和原工作区状态；如继续处理脏文件，只在干净 worktree 中实现并提交，不要直接还原原工作区长期修改。
